...
export default function LostFound() {
  ...
  const handleMarkResolved = async (itemId, resolved) => {
    try {
      const response = await fetch(`http://localhost:5000/api/lostfound/${itemId}`, {
        method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ userId: user.id, resolved })
      });
      let data = {};
      try { data = await response.json(); } catch (_) {}
      if (response.ok) {
            ...
            <div className="detail-modal-info">
              ...
              {detailItem.userId === user.id && (
                <div className="detail-modal-resolve-actions">
                  {detailItem.resolved ? (
                    <button type="button" className="mark-active-button" onClick={() => handleMarkResolved(detailItem.id, false)}>Mark as active again</button>
                  ) : (
                    <button type="button" className="mark-resolved-button" onClick={() => handleMarkResolved(detailItem.id, true)}>Mark as resolved</button>
                  )}
                </div>
              )}
