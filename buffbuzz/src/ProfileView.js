...
export default function ProfileView() {
  ...
  useEffect(() => {
    ...
    const fetchProfile = async () => {
      try {
        setLoading(true);
        const response = await fetch(`http://localhost:5000/api/profile/${targetUserId}?viewerId=${userData.id}`);
        if (response.ok) {
    ...
    (async () => {
      try {
        const [postsRes, friendsRes] = await Promise.all([
          fetch(`http://localhost:5000/api/posts?authorId=${encodeURIComponent(viewingUserId)}&userId=${encodeURIComponent(currentUserId)}`),
  ...
  const handleCancelRequest = async () => {
    ...
    try {
      const response = await fetch(`http://localhost:5000/api/friends/reject/${friendshipId}`, {
        method: 'DELETE', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ userId: currentUserId })
      });
  ...
  const handleRespondToRequest = async (accept) => {
    ...
    setFriendButtonLoading(true);
    try {
      const url = accept ? `http://localhost:5000/api/friends/accept/${friendshipId}` : `http://localhost:5000/api/friends/reject/${friendshipId}`;
      const response = await fetch(url, { method: accept ? 'PUT' : 'DELETE', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ userId: currentUserId }) });
      if (response.ok) {
  ...
  const handleBlock = async () => {
    ...
    try {
      const response = await fetch(`http://localhost:5000/api/block/${viewingUserId}`, {
        method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ blockerId: currentUserId })
      });
  ...
  const handleUnblock = async () => {
    ...
    try {
      const response = await fetch(`http://localhost:5000/api/unblock/${viewingUserId}`, {
        method: 'DELETE', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ blockerId: currentUserId })
      });
